...
def supervisor_brain_prompt(state: ConversationState) -> list[AnyMessage]:
    ...
    """
    system_prompt = """You are the Supervisor Brain for Main Outlet Media.

YOUR COMPLETE WORKFLOW (IN ORDER):
1. Use analyze_and_score_lead to analyze everything
2. Use update_ghl_complete to save all changes
3. Use route_to_agent to send to appropriate agent
...
IMPORTANT: You must complete ALL three steps in order!"""
