"""
FIXED Production Workflow - Simplified and Working
- Added missing run_workflow function
- Only loads messages from GHL (no duplication)
- Redis only stores workflow state, not messages
"""
...
# Use simple memory checkpointer - Redis is overkill for this
...
async def run_workflow(webhook_data: Dict[str, Any]) -> Dict[str, Any]:
    """
    THE MISSING FUNCTION! Run workflow from webhook data
    ...
    try:
        ...
        # Create thread ID from conversation ID (not contact ID)
        thread_id = f"conv-{conversation_id}" if conversation_id else f"contact-{contact_id}"
        
        # Create initial state
        # IMPORTANT: Start with empty messages - receptionist will load from GHL
        initial_state = {
            "messages": [],  # Empty! Let receptionist load from GHL
