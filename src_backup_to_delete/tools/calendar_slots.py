...
def format_slot_for_spanish(slot: Dict[str, Any]) -> str:
    ...
    days_diff = (start_time.date() - now.date()).days
    ...
    if days_diff == 0:
        ...
        return f"pasado mañana a las {time_str}"
    else:
        # Include date for clarity
        day_num = start_time.day
        month_name = months_spanish[start_time.month]
        return f"el {day_name} {day_num} de {month_name} a las {time_str}"
