...
def test_workflow_import():
    ...
    expected_nodes = ["__start__", "receptionist", "intelligence", "supervisor", "responder", "__end__"]
    
    missing_nodes = set(expected_nodes) - set(nodes)
    if missing_nodes:
...
async def test_agents():
    ...
    # Test Maria
    from app.agents.maria_agent_v2 import create_maria_agent
    maria = create_maria_agent()
    agents_tested.append("Maria")
    
    # Test Carlos
    from app.agents.carlos_agent_v2 import create_carlos_agent
    carlos = create_carlos_agent()
    agents_tested.append("Carlos")
    
    # Test Sofia
    from app.agents.sofia_agent_v2 import create_sofia_agent
    sofia = create_sofia_agent()
    agents_tested.append("Sofia")
    
    # Test Supervisor
    from app.agents.supervisor import supervisor_node
    agents_tested.append("Supervisor")
    
    return f"Successfully instantiated: {', '.join(agents_tested)}"
...
def test_base_agent():
    ...
    if current != "I have a restaurant":
        raise ValueError(f"get_current_message failed: {current}")
...
def test_intelligence():
    ...
    test_cases = [
        ("Hola, tengo un restaurante", {"business_type": "restaurante"}),
        ("Mi nombre es Juan", {"name": "Juan"}),
        ("necesito automatizar whatsapp", {"goal": "automatizar whatsapp"}),
        ("Mi presupuesto es como unos $300", {"budget": "300"})
    ]
    
    for text, expected in test_cases:
        result = analyzer.extract_spanish_patterns(text)
        extracted_data = result["extracted_data"]
        
        for key, value in expected.items():
            if key not in extracted_data or extracted_data[key] != value:
                raise ValueError(f"Failed to extract {key} from '{text}'")
    
    # Test scoring
    score_result = analyzer.calculate_lead_score({
        "name": "Juan",
    ...
    if score_result["score"] < 5:
        raise ValueError(f"Score too low for good lead: {score_result['score']}")
    
    return "Extraction and scoring working correctly"
...
async def test_user_journey():
    ...
    state: MinimalState = {
        "messages": [{"role": "user", "content": webhook_data["body"]}],
    ...
    analyzer = IntelligenceAnalyzer()
    intel_result = analyzer.analyze(state)
    
    if intel_result["lead_score"] < 3:
            ...
            with open(file_path, 'r') as f:
                ...
                for pattern in deleted_patterns:
                    if pattern in content and pattern != "ConversationState":  # Allow in comments
                        found_references.append(f"{file_path}: {pattern}")
