...
def test_base_agent():
    ...
    # Test check_score_boundaries with proper parameters
    logger = get_logger("test")
    boundaries = check_score_boundaries(5, 5, 7, "TestAgent", logger)
    if boundaries is not None:  # Should be None for valid score
        raise ValueError("check_score_boundaries failed for valid score")
    
    # Test out of bounds
    boundaries = check_score_boundaries(3, 5, 7, "TestAgent", logger)
    if boundaries is None or not boundaries.get("needs_rerouting"):
        raise ValueError("check_score_boundaries failed for low score")
...
@test("12. Make Validate Command")
def test_make_validate():
    """Test the make validate command works"""
    import subprocess
    ...
    if result.returncode != 0:
        raise ValueError(f"make validate failed: {result.stderr}")
    
    return "make validate passed successfully"
...
"""
    ...
    checklist_items = [
        ...
        ("Make validate passes", any(t["name"].startswith("12.") and t["status"] == "PASS" for t in results))
    ]
