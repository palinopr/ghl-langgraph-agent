    ...
    try:
        ...
        if hasattr(command, 'goto'):
            if command.goto == END or command.goto == 'end':
                filtered_update = filter_agent_result(command.update) if command.update else {}
                return {"should_end": True, **filtered_update}
            else:
                filtered_update = filter_agent_result(command.update) if command.update else {}
                return {"next_agent": command.goto, **filtered_update}
        else:
...
def _merge_agent_results(
    ...
    skip_fields = {"contact_id", "contact_name", "contact_email", "contact_phone", 
                   "conversation_started_at", "webhook_data", "remaining_steps"}
